...
async function testAllStock() {
  try {
    ...
    materials.forEach((m: any) => console.log(`   - ${m.name} (${m.unit})`));
    
    console.log(`\n🏪 Godowns: ${godowns.length}`);
    godowns.forEach((g: any) => console.log(`   - ${g.name}`));
    ...
    for (const godown of godowns) {
      ...
      const activeStock = Object.values(stockLevels).filter((level: any) => level.quantity > 0);
      
      if (activeStock.length === 0) {
        console.log('   No stock available');
      } else {
        activeStock.forEach((level: any) => {
